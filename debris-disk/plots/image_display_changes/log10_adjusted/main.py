...
def smoothed_img():
    ...
    def log10_adjusted(x, mini_value, eps=0.1):
        if x == 0:
            return mini_value - eps
        return np.log10(x)
    ...
    #display = np.power(Imsmooth, 1./4.)
    display = Imsmooth
    mini_val = np.log10(min_nonzero(display))
    print(mini_val)
    for i in range(len(display)):
        for j in range(len(display[i])):
            display[i][j] = log10_adjusted(display[i][j], mini_val)
